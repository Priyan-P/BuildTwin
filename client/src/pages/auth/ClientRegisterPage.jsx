...
const ClientRegisterPage = () => {
  const { clientRegister, authError } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    const res = await clientRegister(name, email, password, phone);
    setSubmitting(false);
    ...
    <div className="min-h-[80vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-6 bg-white p-8 rounded-3xl border border-slate-200 shadow-xl border-t-4 border-t-teal-500">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {submitting ? 'Registering Client...' : 'Register Client Account'}
        ...
        <div className="text-center text-xs text-slate-500 pt-2 border-t border-slate-100">
          ...
          <Link to="/client/login" className="font-bold text-teal-700 hover:underline">
            Client Login
          </Link>
        </div>
